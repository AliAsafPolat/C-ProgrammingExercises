...
int func2(int key,int tablesize){
	return (key*31)%tablesize+1;
}
...
void insert(int *hashtable,int tableSize,int key){
	...
	int addr2=func2(key,tableSize);

	if(hashtable[addr]==-1)
	hashtable[addr]=key;								// hangi adres hangi key e atanmýþ onu görmek istiyorum sadece.
	else{
		i=1;
	while(hashtable[(addr+i*addr2)%tableSize]!=-1&&i<tableSize)			// boþ yer bulana kadar git.
	i++;
	if(i!=tableSize)
	hashtable[(addr+i*addr2)%tableSize]=key;
	else
...
int search(int *hashtable,int tablesize,int key){
	...
	return addr;									// hangi indexte olduðunu döndürsün istiyorum.
		...
		i=1;
		while(hashtable[(addr+i*addr2)%tablesize]!=key&&i<tablesize)
		i++;
		
		if(i==tablesize)							// sürekli aramasýn diye.bulamazsa sonsuz döngüye girecek yoksa.
		return -1;
		else
		return (addr+i*addr2)%tablesize;
	}
}
...
void delete(int *hashtable,int tablesize,int key){
	...
	if(hashtable[addr]==key){
		...
		hashtable[(addr+i*addr2)%tablesize]=-1;
	}
}
