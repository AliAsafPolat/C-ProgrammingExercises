...
int binaryHata(Node *head){					
	int ret;									//Hatanın bulunduğu yeri döndürüyorum.
	int static flag=0;							//Hata bulunduysa flag=1 yapıp çıkıyorum.
	if(head==NULL)								//Eğer rootum null sa dön. Recursive bitiş kontrolüm budur.
	return;
	else{
	 if(head->left!=NULL){						//Eğer sol ayağım boş değilse kontrol et
	 ...
	 if(flag==0&&head->right!=NULL){			//Flag kontrolünü burada yapmamın sebebi ağacın sol tarafına daha önce bakıldığı için 
		if(head->right->deger<head->deger){		//orada bulunan ret değeri burada bozulabiliyor fakat buradan sonra başka kod parçası olmadığından
